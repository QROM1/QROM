...
CHANNELS_STR = "❖ 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 **- 🝢 - احصـائيـات جميـع القنـوات** ❖\n\n"
CHANNELS_ADMINSTR = "❖ 𝙎𝙊𝙐𝙍𝘾𝞝  **- 🝢 - احصـائيـات جميـع القنـوات اشـراف** ❖\n\n"
CHANNELS_OWNERSTR = "❖ 𝙎𝙊𝙐𝙍𝘾𝞝  **- 🝢 - احصـائيـات جميـع القنـوات ملكيـة** ❖\n\n"
GROUPS_STR = "❖ 𝐀𝐍𝐘𝐍𝐌𝐔𝐒  **- 🝢 - احصـائيـات جميـع المجمـوعـات** ❖\n\n"
GROUPS_ADMINSTR = "❖ 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 **- 🝢 - احصـائيـات جميـع المجمـوعـات اشـراف** ❖\n\n"
GROUPS_OWNERSTR = "❖ 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 **- 🝢 - احصـائيـات جميـع المجمـوعـات ملكيـة** ❖\n\n"
# =========================================================== #
#                                                           ZThon                                                                #
# =========================================================== #
...
async def stats(event):  # sourcery no-metrics
    ...
    response = f"❖ 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 **- 🝢 - احصـائيـات {full_name}** ❖\n"
...
@zedub.zed_cmd(
    pattern="قروباتي (الكل|ادمن|مالك)$",
)
async def stats(event):  # sourcery no-metrics
    catcmd = event.pattern_match.group(1)
    zedevent = await edit_or_reply(event, STAT_INDICATION)
...
async def _(event):
    ...
    mentions = "❖ 𝑺𝑶𝑼𝑹𝑪𝑬 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 𝑮𝑹𝑶𝑼𝑷 𝑫𝑨𝑻𝑨 ❖\n** ❖ المشرفـون في ۿذه المجموعه :** \n"
    reply_message = await reply_id(event)
    input_str = event.pattern_match.group(1)
    to_write_chat = await event.get_input_chat()
    chat = None
    if input_str:
        mentions = f"❖ 𝑺𝑶𝑼𝑹𝑪𝑬 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 𝑮𝑹𝑶𝑼𝑷 𝑫𝑨𝑻𝑨 ❖\n** ❖ المشرفـون في {input_str} :** \n"
        try:
...
async def get_chatinfo(event, zedevent):
    ...
    try:
        ...
        try:
            ...
            await zedevent.edit(
                "**❖╎هـذه مجموعـة أو قنـاة خاصـة أو لقد تمّ حظـري منها❖**"
            )
            return None
...
if username is not None:
    caption += f"<b>❖ نوع المجموعـة ⌂:</b>  مجموعـة عامّـة  \n"
    ...
    if creator_username is not None:
        ...
        caption += f'<b>❖ المالـك :</b>  <a href="tg://user?id={creator_id}">{creator_firstname}</a>\n'
    if created is not None:
        caption += f"<b>❖ تاريخ الإنشـاء :</b>  \n <code>{created.date().strftime('%b %d, %Y')} - {created.time()}</code>\n"
    else:
        caption += f"<b>❖ الإنتاج :</b>    <code>{chat_obj_info.date.date().strftime('%b %d, %Y')} - {chat_obj_info.date.time()}</code> {warn_emoji}\n"
    ...
    if members_online:
        caption += f"<b>❖ المتصلين حالياً:</b>  <code>{members_online}</code>\n"
    ...
    caption += "\n"
    if not broadcast:
        caption += f"<b>❖ الوضع البطيء:</b>  {slowmode}"
        if (
    ...
    if hasattr(chat_obj_info, "restricted"):
        ...
        if chat_obj_info.restricted:
            ...
            caption += f"> <b>❖ السبب :</b>  {chat_obj_info.restriction_reason[0].reason}\n"
...
async def count(event):
    ...
    result += f"❖ 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 **- 🝢 - احصـائيـات الحسـاب** ❖\n"

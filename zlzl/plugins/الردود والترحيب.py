...
ZelzalWF_cmd = (
    "❖  𝐀𝐍𝐘𝐍𝐌𝐔𝐒 - **اوامـر الـردود/الترحيب 🎡** ❖\n"
    "**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**\n\n"
...
async def add_new_filter(event):
    ...
    if msg and msg.media and not string:
        if BOTLOG:
            ...
            await edit_or_reply(
                event,
                "**❈╎يتطلب رد ميديـا تعيين قروب السجـل اولاً ..**\n**❈╎لاضافـة قروب السجـل**\n**❖╎اتبـع الشـرح ⇚** https://t.me/ANENMUS",
            )
            return
...
@zedub.zed_cmd(
    pattern="ردود القروب$",
    command=("ردود القروب", plugin_category),
    info={
        "header": "To list all filters in that chat.",
        "الاسـتخـدام": "{tr}ردود القروب",
    },
)
async def on_snip_list(event):
    ...
    for filt in filters:
        if OUT_STR == "** ❖╎لاتوجـد ردود محفوظـه في هـذه الدردشـه ❖**":
            OUT_STR = "❖ 𝗦𝗼𝘂𝗿𝗰𝗲 𝐀𝐍𝐘𝐍𝐌𝐔𝐒 - قائمـة الـردود ❖\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "👉 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def on_all_snip_delete(event):
    ...
    if filters:
        ...
        await edit_or_reply(event, "**❖ تم حذف جـميع الــردود المضـافـه هنـا .. بنجـاح☑️**")
    else:
        await edit_or_reply(event, "**❖ لا توجـد ردود مضـافـه في هـذه المجموعـة**")
...
async def save_welcome(event):
    ...
    if msg and msg.media and not string:
        ...
        string = rep_msg.text
    success = "**❖ {} التـرحيب .. بنجـاح فـي هـذه الدردشـه ❖.**"
...
async def save_welcome(event):
    ...
    if msg and msg.media and not string:
        ...
        rep_msg = await event.get_reply_message()
        string = rep_msg.text
    success = "**❖ تـرحيب الخـاص {}  بنجـاح .. في هذه الدردشـه ❖**"
